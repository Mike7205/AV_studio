...
with tab_rec:
    ...
    if rec_mode == "🎤  Audio only":
        ...
        rec_result = _video_recorder(key="av_rec", default=None)
        if rec_result and isinstance(rec_result, dict) and rec_result.get("done"):
            ext = "mp4" if "mp4" in str(rec_result.get("mime", "")) else "webm"
            mb = rec_result.get("size", 0) / 1_048_576
            st.success(
                f"✅ Nagranie gotowe — pobierz plik z panelu powyżej.  "
                f"Format: {ext.upper()} | Rozmiar: {mb:.1f} MB",
                icon="🎬",
            )
